...
export default function Home() {
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    try {
      ...
      setJobs((prevJobs) =>
        prevJobs.map((job) => {
          const jobId = job.hash_id || job.HashId;
          if (jobId === id) {
  ...
  const getStatusClasses = (status: string) => {
    switch (status?.toLowerCase()) {
      ...
      case "viewed": return "bg-slate-50 text-slate-600 border-slate-200";
  ...
  if (error) return <div className="flex min-h-screen items-center justify-center font-sans text-red-500">Error: {error}</div>;
  ...
  const statusList = ["new", "viewed", "interview", "rejected", "offered"];
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {((filterStatus === "null") ? jobs : jobsFiltered).sort(CMPCostedDate).map((job) => {
                  ...
                  const location = rawLocation.length > 50 ? rawLocation.substring(0, 50) + "..." : rawLocation;
                  ...
                  const allSkills = [
                    ...
                    ...databases.map((s: string) => ({ name: s, type: 'db' }))
                  ].slice(0, 5);
